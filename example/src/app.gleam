            ...
            html([], [
              ...
              html.body([], [
                element(
                  ...
                  [server_component.route("/example")],
...
type Example =
  Subject(lustre.Action(example.Msg, lustre.ServerComponent))

fn socket_init(
  _conn: WebsocketConnection,
) -> #(Example, Option(Selector(lustre.Patch(example.Msg)))) {
  let self = process.new_subject()
  let app = example.app()
  let assert Ok(example) = lustre.start_actor(app, 0)
...
fn socket_update(
  ...
  msg: WebsocketMessage(lustre.Patch(example.Msg)),
) {
  case msg {
    mist.Text(json) -> {
      ...
      case action {
        Ok(action) -> process.send(example, action)
        Error(_) -> Nil
      }

      actor.continue(example)
    }

    mist.Binary(_) -> actor.continue(example)
    mist.Custom(patch) -> {
      let assert Ok(_) =
        patch
...
fn socket_close(example: Example) {
  process.send(example, lustre.shutdown())
}
